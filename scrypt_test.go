	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !tc.wantErr {
				...
				params := parts[0]
				if !strings.Contains(params, "N=") || !strings.Contains(params, "r=") ||
					!strings.Contains(params, "p=") || !strings.Contains(params, "keyLen=") {
...
func TestScryptPasswordEncoder_InvalidFormat(t *testing.T) {
	...
	_, err = encoder.Verify("password", "N=16384,r=8,p=1,keyLen=32$salt")
	if err == nil {
